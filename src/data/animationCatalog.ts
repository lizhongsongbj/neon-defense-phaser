...
const enemies:AnimationCatalogEntry[]=enemySeeds.flatMap(seed=>[
 ...
 {id:`${seed.id}-attack`,category:'enemy',family:seed.family,name:seed.family,level:'攻击',action:seed.id==='gang'?'举枪点射':'攻击',description:seed.id==='gang'?'帮派义体兵压低重心，将步枪从巡逻姿态抬至肩线，完成短促三连点射并承受真实后坐。':seed.id==='riot'?'防暴镇压机降低机械重心，宽脚撑地，以胸前重盾向前冲撞并抬起警棍准备补击。':seed.id==='ninja'?'相位忍者从疾走姿态急停转髋，以低重心完成横向能量刀斩击并迅速回收。':seed.attack,timeline:seed.id==='gang'?'发现目标 → 双手举枪贴肩 → 瞄准锁定 → 三连点射 → 后坐复位 → 放低枪口':seed.id==='riot'?'锁定目标 → 液压下沉 → 重盾前推 → 冲撞命中 → 警棍回摆 → 复位':seed.id==='ninja'?'相位接近 → 前脚落地 → 转髋拔刀 → 水平斩击 → 刀光消散 → 回收架势':'锁定 → 动作蓄力 → 命中 → 收势',referenceImage:seed.image,previewAsset:actionAssets[seed.id],previewFrames:runtimePreviewFrames[seed.id],attackEffectKind:seed.id==='gang'?'gang-rifle':undefined,status:actionAssets[seed.id]||seed.id==='gang'?'available':'planned',motion:'attack',accent:seed.accent,tags:[seed.family,'攻击',...(seed.id==='gang'?['举枪','步枪点射']:[])]},
