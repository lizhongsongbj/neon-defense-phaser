...
const enemies:AnimationCatalogEntry[]=enemySeeds.flatMap(seed=>[
 ...
 {id:`${seed.id}-death`,category:'enemy',family:seed.family,name:seed.family,level:'??',action:'??',description:seed.id==='riot'?'??????????????????????????????????????????????':seed.id==='ninja'?'???????????????????????????????????????':'?????????????????????????',timeline:seed.id==='riot'?'???? ? ???? ? ???? ? ???? ? ????':seed.id==='ninja'?'???? ? ???? ? ???? ? ???? ? ?????':'??? ? ?? ? ????? ? ??',referenceImage:seed.image,previewAsset:deathAssets[seed.id],status:deathAssets[seed.id]?'available':'planned',motion:'death',accent:seed.accent,tags:[seed.family,'??']},
