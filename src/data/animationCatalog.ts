...
const enemies:AnimationCatalogEntry[]=enemySeeds.flatMap(seed=>[
 ...
 {id:`${seed.id}-death`,category:'enemy',family:seed.family,name:seed.family,level:'死亡',action:'死亡',description:seed.id==='riot'?'防暴镇压机液压系统失压，重盾先行坠地，机体跪倒后侧翻，并在地面留下机械零件。':seed.id==='ninja'?'相位忍者的隐身轮廓失稳，能量刀熄灭，身体跪倒后向侧面倒下。':'单位受到致命伤害后失去支撑，完成倒地并留下对应的死亡残留特效。',timeline:seed.id==='riot'?'致命受击 → 液压失压 → 重盾坠地 → 跪倒侧翻 → 零件残留':seed.id==='ninja'?'致命受击 → 相位失稳 → 能量刀熄灭 → 跪倒 → 侧向倒地':'致命受击 → 动作失衡 → 倒地 → 残留出现 → 淡出',referenceImage:seed.image,previewAsset:deathAssets[seed.id],previewFrames:runtimePreviewFrames[seed.id]?.map((url)=>url.replace('/runtime/','/runtime-clean-v4/').replace('/ATTACK/','/death/')),status:deathAssets[seed.id]?'available':'planned',motion:'death',accent:seed.accent,tags:[seed.family,'死亡']},
