...
export function towerUpgradeCost(typeId: AllTowerId, currentLevel: number): number {
  const base = ALL_TOWER_TYPE_BY_ID[typeId].cost
  return Math.round(base * (currentLevel === 1 ? 0.7 : 1.05))
}

/** 拆除塔楼返还的金币,原 `refundValue` */
export function towerRefundValue(typeId: AllTowerId, spent: number): number {
  const base = ALL_TOWER_TYPE_BY_ID[typeId].cost
