...
const result=await page.evaluate(()=>{const names=['企业浮空艇','数据吞噬者','电磁镇暴体','劫持浮游体']; const cards=[...document.querySelectorAll('.animation-card')]; return names.map(name=>{const matches=cards.filter(c=>c.textContent?.includes(name)); return {name,count:matches.length,imgs:matches.map(c=>({src:c.querySelector('img')?.src,tag:c.querySelector('em')?.textContent}))}})})
