...
const root=process.cwd(), rootDir=path.join(root,'public/assets/animations/enemies/runtime')
...
for(const [id,j] of Object.entries(jobs)){for(const motion of ['move','attack','death'])for(let i=1;i<=j.count;i++){const s=String(i).padStart(2,'0'),p=path.join(rootDir,id,motion,`frame-${s}.webp`);await fs.writeFile(p,await clean(p))}console.log(id,'cleaned long thin border lines')}
