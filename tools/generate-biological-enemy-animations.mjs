...
function effectsSvg(kind, enemy, frame) {
  ...
  if (kind === 'move') {
    if (enemy.id.includes('neuro')) return `<svg width="512" height="512"></svg>`
    if (enemy.id.includes('matriarch')) return `<svg width="512" height="512"></svg>`
  ...
  if (enemy.id.includes('neuro') || enemy.id.includes('matriarch')) return `<svg width="512" height="512"><g fill="${enemy.danger}" opacity="${fade * 0.42}"><circle cx="${130 + frame * 13}" cy="${190 + frame * 5}" r="4"/><circle cx="${385 - frame * 9}" cy="${235 + frame * 7}" r="3"/></g></svg>`
